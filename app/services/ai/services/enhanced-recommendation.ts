...
import { EnsembleEngine } from "../core/ensemble-engine";
import { 
  UserPreferences, 
  AIAnalysisResult, 
  RecommendationResult 
	...
	export class EnhancedRecommendationService {
	  ...
	  async analyzeUserNeeds(messages: string[], userId: string): Promise<AIAnalysisResult> {
	    ...
	    const { object: rawObject } = await (generateObject as any)({
    ...
    const object = rawObject as AIAnalysisResult;
    // 更新用户模型
    await this.updateUserModel(userId, object);
	  ...
	  private async scoreIntegrationsAI(
	    ...
	    const scored = await Promise.all(
	      integrations.map(async (integration) => {
	        const { object: rawScoreObject } = await generateObject({
          model: openai("gpt-4o") as any,
          schema: {
            type: "object",
            properties: {
        ...
        const scoreObject = rawScoreObject as any;
        return {
          integration,
          score: scoreObject.relevanceScore,
          reasoning: scoreObject.reasoning
        };
	      })
	    );
	  ...
	  private applyDiversityAlgorithm(
	    ...
	    const adjusted = scoredIntegrations.map(item => {
	      const typeWeight = (diversityWeights as any)[learningStyle]?.[item.integration.type] || 1;
      const complexityWeight = (complexityWeights as any)[technicalLevel]?.[item.integration.complexity] || 1;
	      return {
	        ...item,
